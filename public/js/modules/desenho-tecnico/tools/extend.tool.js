...
export class ExtendTool extends BaseTool {
  ...
  extend(entity, clickPoint) {
    ...
    if (entity.type === 'line' || entity.type === 'centerline') {
      ...
      const i = lineIntersection(p1, p2, b1, b2, { segmentA: false, segmentB: true });
      if (!i) return false;
    ...
    if (entity.type === 'polyline') {
      ...
      const i0 = lineIntersection(pts[0], pts[1], b1, b2, { segmentA: false, segmentB: true });
      const il = lineIntersection(pts[pts.length - 2], pts[pts.length - 1], b1, b2, { segmentA: false, segmentB: true });
