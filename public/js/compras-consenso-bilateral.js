(() => {
  function ensureConsensusStyles() {
    ...
    link.rel = 'stylesheet';
    link.href = '/css/compras-consenso-bilateral.css?v=20260911-2';
  ...
  function updateTopbarNotifications(rows) {
    if (!rows.length) return;
    const wrapper = document.querySelector('.topbar-notification-wrap');
    const counter = wrapper?.querySelector('.topbar-counter');
    const dropdown = wrapper?.querySelector('.topbar-notification-dropdown');
    const summary = wrapper?.querySelector('summary');
    if (!wrapper || !counter || !dropdown || wrapper.dataset.purchaseConsensusLoaded === '1') return;

    wrapper.dataset.purchaseConsensusLoaded = '1';
    const existing = Number(counter.textContent || 0);
    counter.textContent = String(existing + rows.length);
    if (summary) summary.setAttribute('aria-label', 'Notificações do sistema');
    dropdown.querySelector('.topbar-empty-state')?.remove();

    rows.slice(0, 8).forEach((row) => {
      const link = document.createElement('a');
      link.className = 'topbar-notification-item purchase-consensus-notification';
      link.href = `/compras/solicitacoes/${row.solicitacao_id}#consenso-itens`;
      const title = document.createElement('strong');
      title.textContent = `${row.numero} · alteração aguardando Compras`;
      const message = document.createElement('span');
      message.className = 'muted';
      message.textContent = `${row.item_nome}: ${formatQty(row.snapshot?.qtd_solicitada)} → ${formatQty(row.proposta?.qtd_solicitada)} ${row.proposta?.unidade || row.snapshot?.unidade || ''}`.trim();
      link.append(title, document.createElement('br'), message);
  ...
  function renderDashboardNotifications(rows) {
    ...
    if (!dashboard || !rows.length || document.querySelector('.purchase-consensus-notice-panel')) return;
    ensureConsensusStyles();

    const section = document.createElement('section');
    section.className = 'dashboard-card purchase-consensus-notice-panel';
    const head = document.createElement('div');
    head.className = 'purchase-consensus-notice-head';
    const copy = document.createElement('div');
    const kicker = document.createElement('span');
    kicker.className = 'purchase-consensus-kicker';
    kicker.textContent = 'AÇÃO NECESSÁRIA';
    const title = document.createElement('h2');
    title.textContent = 'Alterações de materiais aguardando Compras';
    const text = document.createElement('p');
    text.textContent = 'O solicitante propôs mudanças de quantidade. Revise e confirme antes de efetivar a compra dos itens.';
    copy.append(kicker, title, text);
    const count = document.createElement('strong');
    count.className = 'purchase-change-count';
    count.textContent = String(rows.length);
    head.append(copy, count);
    section.append(head);

    const list = document.createElement('div');
    list.className = 'purchase-consensus-notice-list';
    rows.slice(0, 6).forEach((row) => {
      const link = document.createElement('a');
      link.href = `/compras/solicitacoes/${row.solicitacao_id}#consenso-itens`;
      const main = document.createElement('span');
      const strong = document.createElement('strong');
      strong.textContent = `${row.numero} — ${row.item_nome}`;
      const small = document.createElement('small');
      small.textContent = `${formatQty(row.snapshot?.qtd_solicitada)} → ${formatQty(row.proposta?.qtd_solicitada)} ${row.proposta?.unidade || row.snapshot?.unidade || ''} · ${row.motivo || 'Sem motivo informado'}`.trim();
  ...
  async function loadPurchaseConsensusNotifications() {
    try {
      const response = await fetch('/compras/consenso-itens/pendentes.json', {
        headers: { Accept: 'application/json' },
        credentials: 'same-origin',
      });
      if (!response.ok) return;
      ...
      if (!rows.length) return;
      updateTopbarNotifications(rows);
      renderDashboardNotifications(rows);
    } catch (error) {
      console.warn('[compras-consenso] Falha ao carregar notificações:', error?.message || error);
    }
  }

  function init() {
    ensureConsensusStyles();
    loadPurchaseConsensusNotifications();
    loadItemConsensus();
  }
