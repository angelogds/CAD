(function(){
  ...
  function renderCharts(){
    ...
    const custos=rows('custos_equipamento').slice(0,8);bar('chartCustosEquipamentos',custos.map(x=>x.equipamento_nome),custos.map(x=>x.comprado_centavos),{horizontal:true,label:'Valor comprado',from:'#dff5e7',to:COLORS.green,currency:true,labelMax:34,links:custos.map(x=>x.equipamento_id?`/equipamentos/${x.equipamento_id}`:null)});
    const meses=rows('custos_mes');line('chartCustosMes',meses.map(x=>x.mes),[{label:'Comprado',data:meses.map(x=>x.comprado_centavos),color:COLORS.green,fillFrom:'rgba(21,153,71,.03)',fillTo:'rgba(21,153,71,.20)'},{label:'Recebido',data:meses.map(x=>x.recebido_centavos),color:COLORS.blue,fill:false}],{currency:true});
  }
