(function(){
  ...
  function renderCharts(){
    ...
    const custos=rows('custos_equipamento').slice().sort((a,b)=>Number(b.consumido_centavos||0)-Number(a.consumido_centavos||0)).slice(0,8);bar('chartCustosEquipamentos',custos.map(x=>x.equipamento_nome),custos.map(x=>x.consumido_centavos),{horizontal:true,label:'Valor consumido',from:'#dff5e7',to:COLORS.green,currency:true,labelMax:34,links:custos.map(x=>x.equipamento_id?`/equipamentos/${x.equipamento_id}`:null)});
    const meses=rows('custos_mes');line('chartCustosMes',meses.map(x=>x.mes),[{label:'Comprado',data:meses.map(x=>x.comprado_centavos),color:COLORS.green,fillFrom:'rgba(21,153,71,.03)',fillTo:'rgba(21,153,71,.20)'},{label:'Recebido',data:meses.map(x=>x.recebido_centavos),color:COLORS.blue,fill:false},{label:'Consumido',data:meses.map(x=>x.consumido_centavos),color:COLORS.orange,fill:false}],{currency:true});
  }
  function renderAll(){if(!state.data)return;renderCards();renderTables();renderQuality();renderCharts();const period=$('#periodoResumo');if(period)period.textContent=`Período analisado: ${state.data.filtros.data_inicial} a ${state.data.filtros.data_final}`;const last=$('#lastUpdate');if(last)last.textContent=new Date().toLocaleString('pt-BR');}
