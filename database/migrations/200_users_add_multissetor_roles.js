const NEW_ROLES = ["ENCARREGADO_LOGISTICA", "ENCARREGADO_FRIGORIFICO"];
const TEMP_TABLE = "users_multissetor_roles_tmp";
...
module.exports = function up({ db, tableExists }) {
  ...
  const missingRoles = NEW_ROLES.filter((role) => !createUsersSql.includes(`'${role}'`));
  ...
  if (foreignKeyErrors.length) {
    throw new Error(`Falha de integridade após atualizar roles de users: ${JSON.stringify(foreignKeyErrors)}`);
  }
};
