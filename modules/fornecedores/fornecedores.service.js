...
function dashboard(filters={}) {
  ...
  const recent=db.prepare(`SELECT si.*,COALESCE(f.nome_fantasia,f.nome) fornecedor_nome,s.numero solicitacao_numero,s.os_id,e.nome equipamento_nome FROM solicitacao_itens si JOIN solicitacoes s ON s.id=si.solicitacao_id LEFT JOIN fornecedores f ON f.id=si.fornecedor_id LEFT JOIN equipamentos e ON e.id=s.equipamento_id WHERE si.status_compra='COMPRADO' ORDER BY COALESCE(si.comprado_em,si.updated_at) DESC LIMIT 8`).all();
...
function history(id,q='') { const p={id:Number(id),q:`%${normalize(q)}%`}; return db.prepare(`SELECT si.*,s.numero solicitacao_numero,s.os_id,s.equipamento_id,e.nome equipamento_nome,COALESCE(f.nome_fantasia,f.nome) fornecedor_nome FROM solicitacao_itens si JOIN solicitacoes s ON s.id=si.solicitacao_id LEFT JOIN equipamentos e ON e.id=s.equipamento_id LEFT JOIN fornecedores f ON f.id=si.fornecedor_id WHERE si.fornecedor_id=@id AND (@q='%%' OR lower(COALESCE(si.item_nome,'')||' '||COALESCE(si.item_descricao,'')) LIKE @q) AND (si.status_cotacao='COTADO' OR si.status_compra='COMPRADO') ORDER BY COALESCE(si.comprado_em,si.cotado_em) DESC`).all(p); }
